...
describe("SearchBox", () => {
  ...
  it("matches a partial, case-insensitive query against the real mock data", () => {
    ...
    // and the map are looking at the same data - "Silent Fox" is one of the
    // deterministic titles mockItems.ts's seeded generator always produces.
    render(<SearchBox />);
    fireEvent.change(screen.getByLabelText("Search items"), { target: { value: "silent fox" } });
    expect(screen.getByRole("button", { name: "Silent Fox" })).toBeInTheDocument();
  });
  ...
  it("focuses the chosen item and clears the search on selection", () => {
    ...
    fireEvent.change(screen.getByLabelText("Search items"), { target: { value: "silent fox" } });
    fireEvent.click(screen.getByRole("button", { name: "Silent Fox" }));
